...
export default {
  ...
  /**
     ...
     */
  getRankList (id) {
    return Vue.http.post('/api/rankingList', {id: id})
  },
  ...
  getNovelListByCat (gender, type, major, minor = '', start = 0, limit = 20) {
      var data = {gender: gender, type: type, major: major, minor: minor, start: start, limit: limit};
    // return Vue.http.get('/book/by-categories?gender=' + gender + '&type=' + type + '&major=' + major + '&minor=' + minor + '&start=' + start + '&limit=' + limit)
      return Vue.http.post('/api/book/by-categories', data)
  },
  ...
  /**
     ...
     */
  getBook (bookId) {
    return Vue.http.post('/api/book/', {bookId:bookId})
  },
  ...
  getGenuineSource (bookId) {
    return Vue.http.post('/api/btoc_summary', {bookId:bookId})
  },
  ...
  /**
     ...
     */
  getMixSource (bookId) {
    return Vue.http.post('/api/atoc_summary', {bookId:bookId})
  },
  ...
  getMixChapters (bookId) {
    return Vue.http.post('/api/mix-atoc/', {bookId:bookId})
  },
  ...
  getChapters (sourceId) {
    return Vue.http.post('/api/atoc/', {sourceId:sourceId})
  },
  ...
  getBookChapterContent (chapterUrl) {
    return Vue.http.post('/api/getChapter', {chapterUrl:chapterUrl})
  },
  ...
  autoComplete (searchWord) {
    return Vue.http.post('/api/book/auto-complete' ,{searchWord:searchWord})
  },
  ...
  fuzzySearch (searchWord) {
    return Vue.http.post('/api/book/fuzzy-search' ,{searchWord:searchWord})
  },
  ...
  /**
     ...
     */
  getUpdate (bookList) {
    return Vue.http.post('/api/book_updated',{bookList:bookList.toString()})
  }
